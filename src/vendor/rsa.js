...
module.exports = encode_deviceprint;

function Hashtable() {
  let j = { __indexToValue: [], __indexToKeys: [] };
  let f = [];
  let h = 0;
  let k = this;
  function g(b) {
...
function DomDataCollection(n) {
  ...
  j.recursiveGetAllFunctionNamesUnderElement = function(B, e, A) {
    ...
    let g;
    let x = j.config;
    let D = x.recursion_level;
    let a = x.collection_mode;
  ...
  j.initFunctionsToExclude = function() {
    ...
    let a = j.config.functionsToExclude;
    let b = a.length;
    while (b--) {
      l.put(a[b], '');
    }
  ...
  j.handleSizeLimit = function() {
    ...
    if (g.colllection_mode != 'full' && b.length > v) {
      let c = e.names;
      let d = c.toString();
      let y = b.length - JSON.stringify(c).length + '[]'.length;
      let f = false;
      let w = c.length;
      while (!f) {
        if (a++ == 1000) {
  ...
  j.collectFields = function(b) {
    let r = document.getElementsByTagName(b);
    if (j.dom_data.inputs === undefined) {
      j.dom_data.inputs = [];
    }
    let e = j.dom_data.inputs;
    let g = e.length;
    let a = r.length;
    while (a--) {
      let c = r[a];
...
function IE_FingerPrint() {
  this.deviceprint_browser = function() {
    ...
    t +=
      SEP + navigator.appMinorVersion + SEP + navigator.cpuClass + SEP + navigator.browserLanguage;
...
ProxyCollector.isTimedOut = function(d, e, f) {
  ...
  if (
    e <= 0 &&
    ((!ProxyCollector.internalPingTime && String(d).indexOf('internalPingTimeToSet') != -1) ||
      (!ProxyCollector.externalPingTime && String(d).indexOf('externalPingTimeToSet') != -1))
  ) {
    d.call(this, -1);
    if (f) {
      f.abort();
...
ProxyCollector.doAjax = function(k, l) {
  ...
  }//${k}:${getRandomPort()}/NonExistentImage${getRandomPort()}.gif`;
  let m;
  let n;
  if (typeof XDomainRequest === 'object') {
    ...
    try {
      ...
      m.send();
      setTimeout(function() {
        ProxyCollector.isTimedOut(l, ProxyCollector.PROXY_DETECTION_TIMEOUT - n.duration(), m);
      }, ProxyCollector.TIMEOUT_CHECK_FREQUENCY);
    } catch (o) {
      var j = ProxyCollector.PROXY_DETECTION_TIMEOUT - n.duration();
      ProxyCollector.doAjaxViaImage(j, l, h);
...
ProxyCollector.collect = function() {
  ...
  if (ProxyCollector.externalPingTime == -1 || ProxyCollector.internalPingTime == -1) {
...
function BlackberryLocationCollector() {
  ...
  let k = { aidMode: 2, timeout: 180, relevancy: 120, expiration: 48, alertDebug: false };
  let o = -1;
  let j = 0;
  ...
  this.handlePosition = function() {
    ...
    if (blackberry.location.latitude === 0 && blackberry.location.longitude === 0) {
      ...
      if (b - a <= k.expiration * 60 * 60 * 1000) {
        ...
        alert(
          'New position is not saved. It is expired: ' + (b - a) * 1000 * 60 * 60 + ' hours old',
        );
      }
    }
    if (n !== null) {
  ...
  this.generateGeolocationJSONStruct = function() {
    let b = null;
    if (n !== null) {
      let a = convertTimestampToGMT(n.timestamp);
      b = {
        GeoLocationInfo: [
          { Status: r, Longitude: n.coords.longitude, Latitude: n.coords.latitude, Timestamp: a },
        ],
      };
    } else {
...
function detectFields() {
  let u = 'form';
  let n = 'input';
  let j = document.getElementsByTagName('form');
  let w = j.length;
  let m;
  let v;
  let q = [];
...
function FingerPrint() {
  ...
  this.deviceprint_browser = function() {
    let a = navigator.userAgent.toLowerCase();
    let b = a + SEP + navigator.appVersion + SEP + navigator.platform;
    return b;
  };
  this.deviceprint_software = function() {
    let a = '';
    ...
    let b = '';
    let n = '';
    let s = navigator.plugins;
    let o = navigator.mimeTypes;
    if (s.length > 0) {
      let r = '';
      let u = 'Plugins';
      let m = s.length;
  ...
  this.deviceprint_timezone = function() {
    let a = (new Date().getTimezoneOffset() / 60) * -1;
    let b = new Date();
  ...
  Date.prototype.deviceprint_stdTimezoneOffset = function() {
    let b = new Date(this.getFullYear(), 0, 1);
    let a = new Date(this.getFullYear(), 6, 1);
  ...
  this.deviceprint_language = function() {
    ...
    let k = navigator.browserLanguage;
    let b = navigator.systemLanguage;
    let h = navigator.userLanguage;
  ...
  this.deviceprint_java = function() {
    let a = navigator.javaEnabled() ? 1 : 0;
    return a;
  };
  this.deviceprint_cookie = function() {
    let a = navigator.cookieEnabled ? 1 : 0;
...
function encode_deviceprint() {
  let b = add_deviceprint();
  return urlEncode(b);
}
function decode_deviceprint() {
  let b = encode_deviceprint;
...
function add_deviceprint() {
  ...
  let c = `version=${d.deviceprint_version()}&pm_fpua=${d.deviceprint_browser()}&pm_fpsc=${d.deviceprint_display()}&pm_fpsw=${d.deviceprint_software()}&pm_fptz=${d.deviceprint_timezone()}&pm_fpln=${d.deviceprint_language()}&pm_fpjv=${d.deviceprint_java()}&pm_fpco=${d.deviceprint_cookie()}`;
  ...
  c = `${c}&pm_os=${BrowserDetect.OS}&pm_brmjv=${parseInt(BrowserDetect.version, 10)}&pm_br=${
...
let GEO_LOCATION_DEFAULT_STRUCT = '{"GeoLocationInfo":[{"Status":4}]}';
let geoLocator = null;
let geoLocatorStatus = false;
...
function HTML5LocationCollector() {
  ...
  let h = { accuracy: 100, timeout: 180, relevancy: 120, expiration: 48 };
  ...
  this.handlePosition = function(d) {
    ...
    let b = getTimestampInMillis(d.timestamp);
  ...
  this.generateGeolocationJSONStruct = function() {
    let b = null;
    if (l !== null) {
      let a = convertTimestampToGMT(l.timestamp);
      b = {
        GeoLocationInfo: [
          {
            Status: g,
...
let UIEventCollector = (function() {
  let K = null;
  let N = null;
  let Y = null;
  let M = null;
  let F = ['output_size_limit'];
  O();
  R();
  function O(b) {
  ...
  function X() {
    let a = document.createEvent ? document.createEvent('Event') : document.createEventObject();
    let b = a.timeStamp || new Date();
    b = new Date(b);
    if (b.getYear() > 2100) {
      b = new Date(b / 1000);
    }
  ...
  function H(b) {
    if (b.type == 'keydown') {
      let a = b.which || b.charCode || b.keyCode;
      let c =
  ...
  function Q() {
    J();
    let b = K.elements;
    for (let e = b.size(); e >= 1; e--) {
      let c = b.getByIndex(e);
      let d = c.id();
      let a = document.getElementById(d);
      if (!a) {
        let f = document.getElementsByName(d);
        if (f.length > 0) {
          a = f[0];
        }
      }
  ...
  function S(d) {
    let f = d || window.event;
    let a = d.target;
    if (a.nodeType == 1) {
      let c = a.getElementsByTagName('form');
...
var BrowserDetect = {
  ...
  dataOS: [
    ...
    { string: navigator.userAgent.toLowerCase(), subString: 'android', identity: 'Android' },
